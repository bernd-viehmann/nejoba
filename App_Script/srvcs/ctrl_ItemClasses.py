# ***********************************************************************************************************************************************
# ctrl_ItemDiscuss.py : class for handling comments, that were added to an item
#                       the class manages the creation of new comments on an nejoba-item
#                       see https://docs.google.com/document/d/1EHOCgHt0otWPVMI_z0CpfcBF2Un70LjBTWcHP6i1Bvw/edit 
# 
# 1. get root-element
# 2. check if have a thread or a 'first-item'
# 
#
#
#
...
class ItemBase( mongoDbMgr.mongoMgr ):
    ...
    def storeGeneralBase( self, objectType, objectDetailID, rootElemGUID=System.DBNull.Value, headline=System.DBNull.Value, bodymsg=System.DBNull.Value, parentID=System.DBNull.Value, followerID=System.DBNull.Value, locationID=System.DBNull.Value, taggingLabels=System.DBNull.Value, timeFrom=System.DBNull.Value, timeTo=System.DBNull.Value ):
        try:
            ...
            #
            #
            self.log.w2lgDvlp("ctrl_ItemBase->storeGeneralBase : type of tagging-lables  " + str( type(taggingLabels) ) ) 
            if (taggingLabels != System.DBNull.Value) and (taggingLabels != None):
                for tag in taggingLabels:
                    newRow = self.itemTagsTbl.NewRow();
                    newRow['_ID']          = newObjId        # the mongo_id
                    newRow['_locationID']  = locationID      # the _id of the location
...
class ItemMngr( ItemBase ):
    ...
    - internal functions -
    chainGetIDs            : returns an array with all item.base-_ids of a "what-ever-kind-of"-thread
    '''
    ...
    def hndl_DEBATE_MSG(self)        : 
        try:
            self.log.w2lgDvlp('ItemBase -> handler to process request called : hndl_DEBATE_MSG(self)')
            o = ItemDebateMessage(self.Page)
            return o
        except Exception,e:
            self.log.w2lgError(traceback.format_exc())
    ...
    def loadItem( self, itemBaseID ):
        try:
            ...
            for col in self.itemTbl.Columns:                                        # add maintenance-data from cached collection item.base
                self.data.Add(col.ColumnName , row[col] )
    ...
    def saveItem( self, objectType, rootElemID = None, parentID = None ):
        try:
            ...
            self.log.w2lgDvlp( "selected jobtype :" + objType )
            
            self.detailHndlr = self.objTypeSwitch[ objType ]()     # select the needed type-class
    ...
    # * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
    # setNewFollowerId :  set the follower-ID in the currently last chain-item 
    #
    ...
    # * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
    # findHeaderForRoot :  returns a list of mongo_ids, containing headers of a root-element 
    #
    # parameter : rootElemID            : what is the mongo-ID of the root-element ?
    #             objectTypeOfHeader    : what type of headers we are looking for ?
    #
    # returns   : list with the DataRows matching the query
    #
    # 02.12.2012   - bervie-      initial realese
    # * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
    def findHeaderForRoot(self, rootElemID, objectTypeOfHeader):
        try:
            rows = self.itemTbl.Rows.Find(rootElemID)
            listOfHeaders =  []
            for row in rows:
                if row[1] == int(objectTypeOfHeader) :
                    self.log.w2lgDev(' row found : ' + row[0].ToString() )
                    listOfHeaders.Add( row )
    ...
    # * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
    # getThreatRendered   :  return the html-coded threat of a discussion!
    #                        
    # a discussion has only one thread which is public viewable. every user can add a comment to it. 
    #
    # this function gets this thread for a given root-elem-id and returns the html-data of it
    #
    # parameter  :  root_element_id
    #
    # returns    : data of the thread coded in html
    #
    #
    # 15.01.2013   - bervie-      initial realese
    #
    # * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
    def getThreatRendered( self, rootElemId ):
        try:
            # get the debate-header-element [ object-type : 7 ]
            header = self.itemTbl.Select("_ID = '" + rootElemId + "' AND objectType = 7" )

            html = System.Text.StringBuilder()

            # if we dont have a discussion-hread yet only show the data of the root-element-id
            if len(header) == 0:
                # show root-elem
                rootData = self.loadItem( rootElemId )
                html.Append('<h3>')
                html.Append(rootData['heading'].ToString())
                html.Append('</h3>')
                html.Append(rootData['body'].ToString())
            else:
                # get the root and all available discussion-items
                html.Append( getHtmlOfThread( header[0]['_ID'] ) )
...
# ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##
# classes for handling of the different dataobjects
# 
# ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##     ##

# class Item_JOB_ROOT( ItemBase ) : pass  see jobs_editor.aspx.py
# class Item_DEBATE_ROOT( ItemBase ) : pass
# class ItemDebateMessage( ItemBase ) : pass

#class Item_JOB_HEADER( ItemBase ) : pass
#class Item_JOB_MSG( ItemBase ) : pass
#class Item_JOB_SUBSCRIBER( ItemBase ) : pass
#class Item_DEBATE_HEADER( ItemBase ) : pass
#class Item_DEBATE_SUBSCRIBER( ItemBase ) : pass
...
class Item_JOB_ROOT( ItemBase ) : 
    ...
    def saveItem( self, objectType, rootElemID = None, parentID = None ):
        try:
            ...
            slctType = self.ui.getCtrl('sel_type').SelectedItem.Value
...
# ################################################################################################################################################################################################################################################################################ #
# ################################################################################################################################################################################################################################################################################ #
class Item_DEBATE_ROOT( ItemBase ) :
    ''' 
    Class for managing a debate-root-object  ( a public discussion connected with a location and one or more hashtags )
    objectType-ID = 1 
    ROOT-element for a debate in nejoba. 
    ...
    #
    #
    # 13.01.2013   - bervie-      initial realese
    #
    # * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
    def __init__(self, page):
        try:
            self.objType = 'DEBATE_ROOT'
            ItemBase.__init__(self, page )
            self.log.w2lgDvlp("Item_DEBATE_ROOT.__init__  called " )
    ...
    def loadItem( self, objectDetailID ):
        try:
            ctrlDict = {'collection':'item.DEBATE_ROOT','slctVal':objectDetailID }
            self.readDoc(ctrlDict)
            self.data = ctrlDict['data']
    ...
    def saveItem( self, objectType, rootElemID = None, parentID = None ):
        try:
            ...
            storeDataDct = {'collection':'item.DEBATE_ROOT','slctKey':None,'data': itemData}
            self.newObjId = self.insertDoc(storeDataDct)
...
# ################################################################################################################################################################################################################################################################################ #
# ################################################################################################################################################################################################################################################################################ #
def ItemDebateMessage( ItemMngr ):
    ''' 
    Class for managing a debate-message-object  
    objectType-ID = 12 
    MESSAGE-element for a debate-item in nejoba. HEADER will be added automatically by this class

    '''

    # * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
    # constructor. save;load and display comments
    #
    #
    # parameter : Page :        reference to current webpage
    #             objType :     Type of object that will be handeled
    ...
    def __init__(self, page):
        try:
            self.log.w2lgDvlp("ItemDebateMessage.__init__  called " )
            self.objType = 'DEBATE_MSG'
            ItemBase.__init__(self, page )
    ...
    # * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
    # loadItem by given objectID : comment
    #
    # parameter : objectDetailID :   ID of dataobject in the detail-collection
    #
    # returns string with the data to be rendered (HTML-code)
    ...
    def loadItem( self, objectDetailID ):
        try:
            ctrlDict = {'collection':'item.DEBATE_MSG','slctVal':objectDetailID }
            self.readDoc(ctrlDict)
            dataLoaded = ctrlDict['data']

            html = System.Text.StringBuilder()
            html.Append( result['body'].ToString() )
            html.Append( '<hr/><br/>' )

            dataLoaded['html'] = html.ToString()
            return dataLoaded
    ...
    # * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
    # saveItem : add new comment to the single-comment-thread 
    #
    # parameter:
    #  
    # objectType : what kind of data should be saved
    ...
    # parentID   : no forerunner available for an root-element means we have a None here
    #
    # returns the ID for the new entry in the cache and item.base collection
    #
    #
    # 21.12.2012   - bervie-      initial realese
    #
    ...
    def saveItem( self, objectType, rootElemID = None, parentID = None ):
        try:
            if rootElemID == None:
                self.log.w2lgError('No ROOT_ELEMENT or CHAIN_HEADER given')
                return

            self.log.w2lgDvlp('SAVE-Item for ItemDebateMessage CALLED')

            # get the comment-thread header as center of all comments
            header = self.getDebateThread( rootElemID )
            self.log.w2lgDvlp("ItemDebateMessage: Header _id for adding comments : " + header['_id'].ToString() )

            # find end-element of the chain
            chainEnd = self.getLastInChain( header['_id'] )
            ...
            storeDataDct = {'collection':'item.DEBATE_MSG','slctKey':None,'data': itemData}
            self.newObjId = self.insertDoc(storeDataDct)

            guid = header['_rootElemGUID']                  # we have a new announce which needs a GUID
            parentId = chainEnd['_id']                      # root elemnts never have parents !
            followerId = System.DBNull.Value                # will be updated with first item_id in the chain
    
            # save data to the cache and the item.base collection
            self.storeGeneralBase( self.objType, self.newObjId, guid, '', body, parentId, followerId )

            # update the chain-sequence
            self.setNewFollowerId( parentId, self.newObjId)
