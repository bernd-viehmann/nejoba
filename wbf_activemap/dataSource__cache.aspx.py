  ...
  '''
    ...
    def idxLstByLocatedTags( self ):
        try:
            ...
            self.createCityList()   # create a listz of the neighbourhood into self.LocationList
    ...
    def idxLstByLocation( self ):
        try:
            ...
            for loc in self.LocationList:
                cityToRead = loc[0] 
                if not self.taggs.LocDict.has_key( cityToRead ) : break         # if location is not in the tag-container we do not have to check for it
    ...
    def cutSlice( self ):
        try:
            ...
            if self.CrsCmd != System.String.Empty : 
                ...
                leftStart = 0
            rigthEnd = self.AmntSnd + leftStart
            self.rowIdxList = self.rowIdxList[leftStart : rigthEnd]         # cut the part of the array we need  THIS IS THE MAIN REASON FOR THIS HERE !  !  !  !  !  !  !  !  !  !  !

            # save the last mongo-id we have send also to the client. this information will be used as 
            # starting-point when the next read will be started 
            lastRow = self.appCch.dtSt.Tables["items"].Rows[ self.rowIdxList[-1] ]
            self.CrsCmd =  lastRow['_id'].ToString()

            if rigthEnd >= len(self.rowIdxList) : 
                self.ActvSliceIdx = self.SlicesCached                       # it is possible to cache more than one slice in the cache. this function sets the slice-counter to the first slice after the cache. so we can go on in the database
                self.CrsCmd = 'end_of_data'                                 # currently the end of the iis-cache is the end of the search-function

            #self.log.w2lgDvlp('CachedDataSource.cutSlice( .. ) - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  ' )
            #self.log.w2lgDvlp(' start of the slice                  [leftStart]                 : ' + unicode(leftStart) )
            #self.log.w2lgDvlp(' end   of the slice                  [rigthEnd ]                 : ' + unicode(rigthEnd ) )
            #self.log.w2lgDvlp(' amount of items to load             [self.AmntSnd + leftStart]  : ' + unicode(self.AmntSnd) )
            #self.log.w2lgDvlp(' results found before cutting        [len(self.rowIdxList)]      : ' + unicode( totalyFound ) )
            #self.log.w2lgDvlp(' length of the cutted result-list    [self.rowIdxList]           : ' + unicode( len(self.rowIdxList)) )
            #self.log.w2lgDvlp(' cursor-command (mongoid/string)     [self.CrsCmd]               : ' + unicode( self.CrsCmd     ) )
            #self.log.w2lgDvlp(' slice active after load finished    [ffstMngId]                 : ' + unicode( self.ActvSliceIdx   ) )
            #for dtRowIdx in self.rowIdxList : self.log.w2lgDvlp(' result item in list                                             : ' + unicode( dtRowIdx ) )
            #self.log.w2lgDvlp('CachedDataSource.cutSlice( .. ) - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  ' )
    ...
    def createCityList( self ):
        try:
            ...
            cntry     = callParam[0].ToString()
            postcd    = callParam[1].ToString()
            self.log.w2lgDvlp( 'createCityList parameter :   country : ' + cntry + ' ; postcode : ' + postcd + ' ; areasize (form web.config ) : ' + areaSize )
            
            self.LocationList = self.geoSrc.getPlacesByPostcode( cntry, postcd, areaSize )                      # define the list of cities in the neighbourhood
