...
def HandlBtnClick(sender, e):
    try:
        btnIdfyr =  sender.ClientID.ToString()
        parts = btnIdfyr.split('_')
        ctrlName     = parts[3]
        reprtId      = parts[4]
        tool.log.w2lgDvlp('selectlist_initiatives -> HandlBtnClick : BUTTON sender.ClientID pressed :  ' + str(btnIdfyr) )

        table = Page.ViewState['TABLE']

        if table is None : return

        if ctrlName == 'btn_EditInitiative':
            tool.log.w2lgDvlp('selectlist_initiatives -> HandlBtnClick : edit   clicked for :  ' + str(reprtId) )
            pass

        elif ctrlName == 'btn_DeleteInitiative':
            tool.log.w2lgDvlp('selectlist_initiatives -> HandlBtnClick : delete clicked for :  ' + str(reprtId) )
            pass
...
def loadData():
        try:
            ...
            col = table.Columns.Add("marker_line"    , type("String") )

            collection = njbDb.GetCollection('initiatives.final')
            query = QueryDocument( 'creator_GUID', guid )
            for initiatives in collection.Find(query) :

                #extract the html-representation of an item
                html = ''
                if len(initiatives['marker_line' ].ToString()) > 0:
                    html = initiatives['marker_line'   ].ToString().split('\t')[2]

                    row = table.NewRow()
                    row['_id']           = initiatives['_id'           ].ToString()
                    row['location_key']  = initiatives['location_key'  ].ToString()
                    row['type']          = initiatives['type'          ].ToString()
                    row['email']         = initiatives['txbx_email'    ].ToString()
                    row['nickname']      = initiatives['txbx_nickname' ].ToString()
                    #row['link']          = humans['txbx_website'  ].ToString()
                    row['picturl']       = initiatives['txbx_picturl'  ].ToString()
                    row['lat']           = initiatives['txbx_lat'      ].ToString()
                    row['lon']           = initiatives['txbx_lon'      ].ToString()
                    row['postcode']      = initiatives['txbx_postcode' ].ToString()
                    row['marker_line']   = html

                    table.Rows.Add(row)
                    tool.log.w2lgDvlp('list_map_data -> loadData : ' + initiatives['txbx_nickname'].ToString() )

            repeater = tool.ui.getCtrl('repInitiativesList')
            repeater.DataSource = table
            repeater.DataBind()

            Page.ViewState['TABLE'] = table
